...
namespace Wallet
{
    ...
    public:
        ...
        bool ReadTx(uint512 hash, CWalletTx& wtx)
        {
            return Read(std::make_pair(std::string("tx"), hash), wtx);
        }

        bool WriteTx(uint512 hash, const CWalletTx& wtx)
        {
            nWalletDBUpdated++;
            return Write(std::make_pair(std::string("tx"), hash), wtx);
        }

        bool EraseTx(uint512 hash)
        {
            nWalletDBUpdated++;
        ...
        bool ReadCScript(const uint256 &hash, CScript& redeemScript)
        {
            redeemScript.clear();
