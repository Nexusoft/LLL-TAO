...
namespace Legacy
{
    ...
    bool Transaction::CoinstakeReward(const TAO::Ledger::BlockState& block, uint64_t& nStakeReward) const
    {
        ...
        for(uint32_t nIndex = 1; nIndex < nInSize; ++nIndex)
        {
            ...
            if(!LLD::legDB->ReadBlock(vin[nIndex].prevout.hash, statePrev))
            {
                ...
                if(!LLD::legDB->RepairIndex(vin[nIndex].prevout.hash, statePrev))
    ...
    bool Transaction::Connect(const std::map<uint512_t, Transaction>& inputs, TAO::Ledger::BlockState& state, uint8_t nFlags) const
    {
        ...
        if (fIsCoinStake || fIsCoinBase)
        {
            ...
            else if(IsTrust())
            {
                ...
                if(!LLD::trustDB->ReadTrustKey(cKey, trustKey))
                {
                    /* When trust key not found, search for genesis. This will assign trustKey too if genesis found. */
        ...
        for (uint32_t i = (uint32_t)fIsCoinStake; i < nInSize; ++i)
        {
            ...
            if (txPrev.IsCoinBase() || txPrev.IsCoinStake())
            {
                ...
                if(!LLD::legDB->ReadBlock(txPrev.GetHash(), statePrev))
                {
                    ...
                    statePrev = state;
                    if(!LLD::legDB->RepairIndex(txPrev.GetHash(), statePrev))
                        return debug::error(FUNCTION, "failed to read previous tx block");
                }
        ...
        if (fIsCoinStake)
        {
            ...
            if (vout[0].nValue / 1000 > (nStakeReward + nValueIn) / 1000)
                return debug::error(FUNCTION, GetHash().ToString().substr(0,10), " stake reward ", vout[0].nValue / 1000, " mismatch ", (nStakeReward + nValueIn) / 1000);
        }
    ...
    bool Transaction::CheckTrust(const TAO::Ledger::BlockState& state, const TAO::Ledger::TrustKey& trustKey) const
    {
        ...
        if(keyLast != cKey)
            return debug::error(FUNCTION,
                "trust key in previous stake block ", cKey.ToString().substr(0, 20),
                " does not match current stake block ", keyLast.ToString().substr(0, 20));
