...
namespace Legacy
{
    ...
    bool CreateLegacyBlock(const Legacy::Wallet::CReserveKey& coinbaseKey, const uint32_t nChannel, const uint32_t nID, LegacyBlock& newBlock)
    {
        ...
        BlockState& prevBlockState = TAO::Ledger::ChainState.stateBest;

        /* Modulate the Block Versions if they correspond to their proper time stamp */
        if (runtime::unifiedtimestamp() >= (fTestNet ? TESTNET_VERSION_TIMELOCK[TESTNET_BLOCK_CURRENT_VERSION - 2] : NETWORK_VERSION_TIMELOCK[NETWORK_BLOCK_CURRENT_VERSION - 2]))
            newBlock.nVersion = fTestNet ? TESTNET_BLOCK_CURRENT_VERSION : NETWORK_BLOCK_CURRENT_VERSION; // --> New Block Version Activation Switch
        else
            newBlock.nVersion = fTestNet ? TESTNET_BLOCK_CURRENT_VERSION - 1 : NETWORK_BLOCK_CURRENT_VERSION - 1;
        ...
        else
        {
            ...
            if (!CreateCoinbaseTransaction(txNew, coinbaseKey, nId, newBlock.nVersion))
            ...
            if (!AddTransactions(newBlock.vtx))
                return debug::error(FUNCTION, "Error adding transactions to block");
        }
...
//TODO -- Add pool support for coinbase
        ...
        NexusAddress ambassadorKeyAddress(fTestNet ? (nNewBlockVersion < 5 ? TESTNET_DUMMY_ADDRESS : TESTNET_DUMMY_AMBASSADOR_RECYCLED) : (nNewBlockVersion < 5 ? CHANNEL_ADDRESSES[nCoinbaseCounter] : AMBASSADOR_ADDRESSES_RECYCLED[nCoinbaseCounter]));
        NexusAddress devKeyAddress(fTestNet ? (nNewBlockVersion < 5 ? TESTNET_DUMMY_ADDRESS : TESTNET_DUMMY_DEVELOPER_RECYCLED) : (nNewBlockVersion < 5 ? DEVELOPER_ADDRESSES[nCoinbaseCounter] : DEVELOPER_ADDRESSES_RECYCLED[nCoinbaseCounter]));
    ...
    void AddTransactions(std::vector<Transaction>& vtx)
    {
        ...
        for (const uint512_t& txHash : vMemPoolHashes)
        {
            ...
            for (const CTxIn& txin : tx.vin)
            {
                ...
                dPriority += (double)nValueIn * (double)std::min(1, txAge);
            ...
            mapPriority.emplace(make_pair(-dPriority, &tx));
        ...
        for (Transaction& tx : vRemoveFromPool)
        {
