...
namespace Legacy
{
    ...
    *   Fills the keypool, requires wallet passphrase to be set */
    encoding::json RPC::KeypoolRefill(const encoding::json& params, const bool fHelp)
    {
        ...
        if(Legacy::Wallet::GetInstance().GetKeyPool().GetKeyPoolSize() < config::GetArg("-keypool", 100))
            throw TAO::API::APIException(-4, "Error refreshing keypool.");
    ...
    *  mintonly is optional true/false allowing only block minting. timeout is ignored if mintonly is true / 1*/
    encoding::json RPC::WalletPassphrase(const encoding::json& params, const bool fHelp)
    {
        ...
        if(!Legacy::Wallet::GetInstance().IsCrypted())
            throw TAO::API::APIException(-15, "Error: running with an unencrypted wallet, but walletpassphrase was called.");

        if(!Legacy::Wallet::GetInstance().IsLocked())
            throw TAO::API::APIException(-17, "Error: Wallet is already unlocked, use walletlock first if need to change unlock settings.");
    ...
    *  Changes the wallet passphrase from <oldpassphrase> to <newpassphrase>*/
    encoding::json RPC::WalletPassphraseChange(const encoding::json& params, const bool fHelp)
    {
        ...
        if(!Legacy::Wallet::GetInstance().IsCrypted())
            throw TAO::API::APIException(-15, "Error: running with an unencrypted wallet, but walletpassphrasechange was called.");
    ...
    *  before being able to call any methods which require the wallet to be unlocked */
    encoding::json RPC::WalletLock(const encoding::json& params, const bool fHelp)
    {
        ...
        if(!Legacy::Wallet::GetInstance().IsCrypted())
            throw TAO::API::APIException(-15, "Error: running with an unencrypted wallet, but walletlock was called.");
    ...
    *  Encrypts the wallet with <passphrase>. */
    encoding::json RPC::EncryptWallet(const encoding::json& params, const bool fHelp)
    {
        ...
        if(Legacy::Wallet::GetInstance().IsCrypted())
            throw TAO::API::APIException(-15, "Error: running with an encrypted wallet, but encryptwallet was called.");
        ...
        if(!Legacy::Wallet::GetInstance().EncryptWallet(strWalletPass))
            throw TAO::API::APIException(-16, "Error: Failed to encrypt the wallet.");
    ...
    *  Adds a private key (as returned by dumpprivkey) to your wallet */
    encoding::json RPC::ImportPrivKey(const encoding::json& params, const bool fHelp)
    {
        ...
        if(!fGood)
            throw TAO::API::APIException(-5,"Invalid private key");

        if(Legacy::Wallet::GetInstance().IsLocked())
            throw TAO::API::APIException(-13, "Error: Please enter the wallet passphrase with walletpassphrase first.");

        if(Legacy::fWalletUnlockMintOnly)
            throw TAO::API::APIException(-102, "Wallet is unlocked for minting only.");
...

        {
            ...
            if(!Legacy::Wallet::GetInstance().AddKey(key))
                throw TAO::API::APIException(-4,"Error adding key to wallet");
        }
    ...
    *  Reveals the private key corresponding to <NexusAddress> */
    encoding::json RPC::DumpPrivKey(const encoding::json& params, const bool fHelp)
    {
        ...
        if(Legacy::Wallet::GetInstance().IsLocked())
            throw TAO::API::APIException(-13, "Error: Please unlock the wallet with walletpassphrase first.");
        ...
        if(!Legacy::Wallet::GetInstance().GetSecret(address, vchSecret, fCompressed))
            throw TAO::API::APIException(-4,"Private key for address " + strAddress + " is not known");
    ...
    *  You need to list the imported keys in a JSON array of {[account],[privatekey]} */
    encoding::json RPC::ImportKeys(const encoding::json& params, const bool fHelp)
    {
        ...
        if(!params[0].is_object())
            throw TAO::API::APIException(-8, "Invalid import list format.");
    ...
    encoding::json RPC::ExportKeys(const encoding::json& params, const bool fHelp)
    {
        ...
        for(std::map<Legacy::NexusAddress, int64_t>::iterator it = mapAddresses.begin(); it != mapAddresses.end(); ++it)
        {
            ...
            if(!Legacy::Wallet::GetInstance().GetSecret(it->first, vchSecret, fCompressed))
                throw TAO::API::APIException(-4,"Private key for address " + it->first.ToString() + " is not known");
