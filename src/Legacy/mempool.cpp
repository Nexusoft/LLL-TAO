            ...
            if(!tx.FetchInputs(inputs))
            {
                ...
                for(const auto& vin : tx.vin)
                {
                    ...
                    if(!fExists)
                    {
                        ...
                        ssResponse << uint8_t(LLP::TritiumNode::TYPES::TRANSACTION) << vin.prevout.hash; // 100% certainty what input tx type is
                        ssResponse << uint8_t(LLP::TritiumNode::SPECIFIER::LEGACY) << uint8_t(LLP::TritiumNode::TYPES::TRANSACTION) << vin.prevout.hash;
                ...
                if(nTotal > 0)
                {
                    ...
                    {
                        /* Push to stream. */
                        ssResponse << uint8_t(LLP::TritiumNode::SPECIFIER::LEGACY) << uint8_t(LLP::TritiumNode::TYPES::TRANSACTION) << hashTx;
