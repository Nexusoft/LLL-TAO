        ...
        for(uint32_t n = 0; n < tx.Size(); ++n)
        {
            ...
            switch(nOP)
            {
                ...
                case TAO::Operation::OP::TRANSFER:
                {
                    ...
                    {
                        LLD::Logical->WriteTransfer(tx.hashGenesis, hashRegister);
                        return;
                    }
            ...
            if(!LLD::Logical->PushRegister(tx.hashGenesis, hashRegister))
            {
                debug::warning(FUNCTION, "failed to push register ", VARIABLE(tx.hashGenesis.SubString()));
                continue;
            }
        }
    }
