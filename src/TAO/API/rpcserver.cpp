...
namespace Net
{
    ...
    void ThreadRPCServer(void* parg)
    {
        ...
        debug::log(0, "ThreadRPCServer exiting\n");
    }

    void ThreadRPCServer2(void* parg)
    {
        debug::log(0, "ThreadRPCServer started\n");
        ...
        if (fUseSSL)
        {
            ...
            else debug::log(0, "ThreadRPCServer ERROR: missing server certificate file %s\n", pathCertFile.string().c_str());

            filesystem::path pathPKFile(GetArg("-rpcsslprivatekeyfile", "server.pem"));
            if (!pathPKFile.is_complete()) pathPKFile = filesystem::path(GetDataDir()) / pathPKFile;
            if (filesystem::exists(pathPKFile)) context.use_private_key_file(pathPKFile.string(), ssl::context::pem);
            else debug::log(0, "ThreadRPCServer ERROR: missing server private key file %s\n", pathPKFile.string().c_str());
        ...
        loop() {
            ...
            if (!ClientAllowed(peer.address().to_string()))
            {
                ...
                debug::log(0, "[RPC] Failed to Authorize New Connection.");
                continue;
            }
            ...
            {   // Timed out:
                acceptor.cancel();
                debug::log(0, "ThreadRPCServer ReadHTTP timeout\n");
                continue;
            }
            ...
            {
                debug::log(0, "ThreadRPCServer incorrect password attempt from %s\n",peer.address().to_string().c_str());
            ...
            try
            {
                ...
                debug::log(0, "ThreadRPCServer method=%s\n", strMethod.c_str());
