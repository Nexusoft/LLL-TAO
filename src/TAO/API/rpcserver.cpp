...
namespace Net
{
    ...
    Value getnetworktrustkeys(const Array& params, bool fHelp)
    {
        ...
        Array trustkeys;
        Object ret;
        for(std::map<uint576_t, Core::CTrustKey>::iterator it = Core::cTrustPool.mapTrustKeys.begin(); it != Core::cTrustPool.mapTrustKeys.end(); ++it)
        {
            Object obj;
    ...
    Wallet::NexusAddress GetAccountAddress(string strAccount, bool bForceNew=false)
    {
        ...
        if (!account.vchPubKey.empty())
        {
            ...
            for (map<uint512_t, Wallet::CWalletTx>::iterator it = pwalletMain->mapWallet.begin();
    ...
    Value getreceivedbyaddress(const Array& params, bool fHelp)
    {
        ...
        // Tally
        int64_t nAmount = 0;
        for (map<uint512_t, Wallet::CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
        {
            const Wallet::CWalletTx& wtx = (*it).second;
    ...
    Value dumprichlist(const Array& params, bool fHelp)
    {
        ...
        multimap<uint64_t, uint256_t> richList = flip_map(Core::mapAddressTransactions);

        /** Dump the Address and Values. **/
        Object entry;
        for(multimap<uint64_t, uint256_t>::const_reverse_iterator it = richList.rbegin(); it != richList.rend() && nCount > 0; ++it)
        {
    ...
    Value gettransactions(const Array& params, bool fHelp)
    {
        ...
        std::vector<std::pair<bool, uint512_t> > vTransactions = Core::mapRichList[cAddress.GetHash256()];
    ...
    Value addmultisigaddress(const Array& params, bool fHelp)
    {
        ...
        uint256_t scriptHash = SK256(inner);
        Wallet::CScript scriptPubKey;
    ...
    Value listtrustkeys(const Array& params, bool fHelp)
    {
        ...
        Object result;
        for(std::map<uint576_t, Core::CTrustKey>::iterator i = Core::cTrustPool.mapTrustKeys.begin(); i != Core::cTrustPool.mapTrustKeys.end(); ++i)
        {
    ...
    Value getrawmempool(const Array& params, bool fHelp)
    {
        ...
        vector<uint512_t> vtxid;
        Core::mempool.queryHashes(vtxid);

        Array a;
        BOOST_FOREACH(const uint512_t& hash, vtxid)
