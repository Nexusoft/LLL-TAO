...
namespace TAO
{
    ...
    namespace API
    {
        ...
        encoding::json Ledger::BlockHash(const encoding::json& params, const bool fHelp)
        {
        ...
        encoding::json Ledger::Blocks(const encoding::json& params, const bool fHelp)
        {
            /* Check for the block height parameter. */
            if(params.find("hash") == params.end() && params.find("height") == params.end())
                throw APIException(-84, "Missing hash or height");
            ...
            while(!blockState.IsNull())
            {
                ...
                encoding::json jsonBlock = TAO::API::BlockToJSON(blockToAdd, nVerbose);
