        ...
        for(const auto& strName : vFieldNames)
        {
            ...
            switch(nType)
            {
                ...
                case TAO::Register::TYPES::STRING:
                {
                    ...
                    /* Set the return value from object register data. */
                    const std::string strValue = strRet.substr(0, strRet.find_last_not_of('\0') + 1);
                    jRet[strName] = strValue;

                    /* Specific rule for names. */
                    if(strName == "name")
                    {
                    ...
                    else if(strName == "namespace")
                    {
                        /* Set the flag for global names. */
                        jRet["global"]    = (strValue == TAO::Register::NAMESPACE::GLOBAL);

                        /* Grab our namespace register address since we derive deterministically. */
                        if(!jRet["global"])
                            jRet["address"] = TAO::Register::Address(strValue, TAO::Register::Address::NAMESPACE).ToString();
                    }
