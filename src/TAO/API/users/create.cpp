...
namespace TAO::API
{
    ...
    encoding::json Users::Create(const encoding::json& jParams, const bool fHelp)
    {
        /* JSON return value. */
        encoding::json ret;
        ...
        const SecureString strUsername =
            jParams["username"].get<std::string>().c_str();

        /* Extract the password */
        const SecureString strPassword =
            jParams["password"].get<std::string>().c_str();
                               ...
                               const SecureString& strPin, TAO::Ledger::Transaction& tx)
    {
        ...
        const uint32_t nContract = tx.Size();
        tx[nContract] << uint8_t(TAO::Operation::OP::CREATE)      << hashRegister
