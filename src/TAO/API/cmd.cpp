...
namespace TAO
{
    ...
    namespace API
    {
        ...
        int CommandLineAPI(int argc, char** argv, int argn)
        {
            ...
            uint16_t nPort = static_cast<uint16_t>(config::GetArg(std::string("-apiport"), config::fTestNet ? TESTNET_API_PORT : MAINNET_API_PORT));
        ...
        int CommandLineRPC(int argc, char** argv, int argn)
        {
            ...
            uint16_t nPort = static_cast<uint16_t>(config::GetArg(std::string("-rpcport"), config::fTestNet ? TESTNET_RPC_PORT : MAINNET_RPC_PORT));
