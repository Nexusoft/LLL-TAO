...
namespace Core
{
    ...
    bool AddOrphanTx(const CDataStream& vMsg)
    {
        ...
        if (pvMsg->size() > 5000)
        {
            debug::log("ignoring large orphan tx (size: %u, hash: %s)\n", pvMsg->size(), hash.ToString().substr(0,10).c_str());
            delete pvMsg;
            return false;
        }
        ...
        debug::log("stored orphan tx %s (mapsz %u)\n", hash.ToString().substr(0,10).c_str(),
    ...
    int CMerkleTx::SetMerkleBranch(const CBlock* pblock)
    {
        ...
        else
        {
            ...
            if (nIndex == (int)pblock->vtx.size())
            {
                vMerkleBranch.clear();
                nIndex = -1;
                debug::log("ERROR: SetMerkleBranch() : couldn't find tx in block\n");
                return 0;
            }
                            ...
                            bool* pfMissingInputs)
    {
        ...
        if (fCheckInputs)
        {
            ...
            if (nFees < MIN_RELAY_TX_FEE)
            {
...

                {
                    ...
                    if (fDebug)
                        debug::log("Rate limit dFreeCount: %g => %g\n", dFreeCount, dFreeCount+nSize);
                    dFreeCount += nSize;
                }
            }
        ...
        // Store transaction in memory
        {
            ...
            if (ptxOld)
            {
                debug::log("CTxMemPool::accept() : replacing tx %s with new version\n", ptxOld->GetHash().ToString().c_str());
                remove(*ptxOld);
            }
            addUnchecked(tx);
        }
        ...
        debug::log("CTxMemPool::accept() : accepted %s\n", hash.ToString().substr(0,10).c_str());
        return true;
    }
    ...
    {
        debug::log("addUnchecked(): size %lu\n",  mapTx.size());
