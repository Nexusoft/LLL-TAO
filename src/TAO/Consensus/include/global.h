...
namespace Core
{
	...
	/* Standard list of Ambassador Addresses sent as second to last input on every transaction (NOTE: Tritium does not use these addresses anymore). */
	extern const std::string AMBASSADOR_ADDRESSES[];
	
	
	/* Standard list of Developer Addresses sent as second to last input on every transaction (NOTE: Tritium does not use these addresses anymore). */
	extern const std::string DEVELOPER_ADDRESSES[];
	
	
	/* Byte Level data of each Developer Script Signature for verifying addresses in coinbase inputs (NOTE: Tritium does not use these anymore). */
	...
	/* The number calculated on a per block basis to determine the most trusted chain (NOTE: Tritium ++ will include this as well as computational work weighted together). */
	...
	/* Memory Only Map of the blocks that have been stored as an Index Object. Used for quick reading / writing of block data. (NOTE: Tritium ++ will eliminate this need). */
	...
	/* Mutable Transaction Fee inherited by constants and mutated by TESTNET switch (TODO: Deprecate Post Tritium). */
	...
	/* The Official Genesis Block switched by the TESTNET flag. (TODO: Deprecate Post Tritium). */
