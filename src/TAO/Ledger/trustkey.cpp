...
namespace TAO::Ledger
{
        ...
        CTrustKey::CTrustKey(std::vector<uint8_t> vchPubKeyIn, uint1024_t hashBlockIn, uint512_t hashTxIn, int32_t nTimeIn)
        {
            SetNull();
        ...
        uint64_t CTrustKey::GetAge(const uint32_t nTime) const
		{ 
        ...
        uint64_t CTrustKey::GetBlockAge(const uint32_t nTime) const
	    {
//TODO requires implementation
		...
		bool CTrustKey::IsExpired(uint32_t nTime) const
		{
        ...
        bool CTrustKey::CheckGenesis(TritiumBlock block) const
        {
            /* Invalid if Null. */
            if(IsNull())
                return false;
            ...
            if(!block.IsProofOfStake())
                return debug::error("CTrustKey::CheckGenesis() : genesis has to be proof of stake");

            /* Trust Key Timestamp must be the same as Genesis Key Block Timestamp. */
            if(nGenesisTime != block.nTime)
                return debug::error("CTrustKey::CheckGenesis() : genesis time mismatch");

            /* Check the genesis block hash. */
            if(block.GetHash() != hashGenesisBlock)
                return debug::error("CTrustKey::CheckGenesis() : genesis hash mismatch");

            /* Genesis Transaction must match Trust Key Genesis Hash. */
            if (block.vtx.size() == 0)
                return debug::error("CTrustKey::CheckGenesis() : genesis coinstake not present");

            uint512_t txHash = block.vtx[0].second;

            if(txHash != hashGenesisTx)
                return debug::error("CTrustKey::CheckGenesis() : genesis coinstake hash mismatch");
        ...
        void CTrustKey::Print()
        {
            debug::log(0, "CTrustKey(%s)", ToString().c_str());
        }
