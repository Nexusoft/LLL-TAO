...
namespace LLP
{
    ...
    bool StatelessMinerConnection::ProcessPacket()
    {
        try
        {
            ...
            /* ────────────────────────────────────────────────────────────────
             * Per-opcode MUTEX isolation (PR #410 — MUTEX optimization).
             *
             * Each handler below acquires its own scoped { LOCK(MUTEX); ... }
             * block only for the specific context fields it needs — typically a
             * brief snapshot at the top and a brief mutation at the bottom.
             *
             * This replaces the former broad LOCK(MUTEX) that covered ALL 16-bit
             * handlers and required 7 manual lk.unlock()/lk.lock() workaround
             * sites to avoid deadlock with new_block(), SendChannelNotification(),
             * and SendStatelessTemplate() (all of which acquire MUTEX internally).
             *
             * Reference pattern: STATELESS_MINER_READY handler (lines ~773–958)
             * already uses per-handler scoped locks successfully.
             * ──────────────────────────────────────────────────────────────────── */
            ...
            if(PACKET.HEADER == GET_BLOCK)
            {
                 ...
                 *
                 * Cache check: If TryAttachBlockTemplate() (PUSH path) already built a
                 * template for the current unified height, reuse its block bytes to avoid
                 * a redundant new_block() call and the associated LLD I/O. */
                ...
                /* Check global template cache before calling new_block() */
                const uint32_t nCurrentUnifiedHeight =
                    static_cast<uint32_t>(TAO::Ledger::ChainState::nBestHeight.load());
                auto gbCached = GlobalTemplateCache::Get().GetCachedTemplate(
                    nChannel_snap, nCurrentUnifiedHeight);

                /* Build request for the stateless lane handler.
                 * If the cache has a valid entry, wrap it in fnCreateBlock so the
                 * handler can use it without modification.  On a cache miss, fall
                 * through to the normal new_block() path. */
                StatelessGetBlockRequest gbReq;
                gbReq.context = ctxSnap;
                if(gbCached.fValid)
                {
                    /* Cache HIT: create a synthetic block factory that skips new_block().
                     * We pass the cached block bytes via StatelessGetBlockHandler by
                     * checking for a pre-built result instead. */
                    debug::log(2, FUNCTION, "GET_BLOCK: cache HIT channel=", nChannel_snap,
                               " unified=", nCurrentUnifiedHeight, " — skipping new_block()");

                    /* Build the 228-byte payload directly from cached data */
                    const uint32_t nCachedUnified = gbCached.nUnifiedHeight;
                    uint32_t nCacheChanHeight = 0;
                    {
                        TAO::Ledger::BlockState stateChanGB = TAO::Ledger::ChainState::tStateBest.load();
                        if(TAO::Ledger::GetLastState(stateChanGB, nChannel_snap))
                            nCacheChanHeight = stateChanGB.nChannelHeight;
                    }
                    std::vector<uint8_t> vCacheMetadata = RoundStateUtility::SerializeTemplateMetadata(
                        nCachedUnified, nCacheChanHeight, gbCached.nBits);

                    StatelessGetBlockResult gbResult;
                    gbResult.fSuccess      = true;
                    gbResult.vPayload.reserve(vCacheMetadata.size() + gbCached.vBlockData.size());
                    gbResult.vPayload.insert(gbResult.vPayload.end(), vCacheMetadata.begin(), vCacheMetadata.end());
                    gbResult.vPayload.insert(gbResult.vPayload.end(), gbCached.vBlockData.begin(), gbCached.vBlockData.end());
                    ...
                    /* Jump straight to send path, bypassing handler */
                    SendGetBlockDataResponse(gbResult.vPayload, true);
                    debug::log(2, "📥 === GET_BLOCK: SUCCESS (cache) ===");

                    /* Update context anchors */
                    {
                        TAO::Ledger::BlockState stateBestGB = TAO::Ledger::ChainState::tStateBest.load();
                        LOCK(MUTEX);
                        context = context.WithTimestamp(runtime::unifiedtimestamp())
                                         .WithHeight(stateBestGB.nHeight)
                                         .WithLastTemplateUnifiedHeight(stateBestGB.nHeight)
                                         .WithHashLastBlock(TAO::Ledger::ChainState::hashBestChain.load());
                    }

                    /* Reset consecutive rate-limit strikes on success */
                    {
                        LOCK(MUTEX);
                    ...
                    /* Atomic transform */
                    {
                        const uint32_t nUH_gb = nCurrentUnifiedHeight;
                        const uint1024_t hashBest_gb = TAO::Ledger::ChainState::hashBestChain.load();
                        StatelessMinerManager::Get().TransformMiner(ctxSnap.strAddress,
                            [nUH_gb, hashBest_gb](const MiningContext& current) {
                                return current.WithTimestamp(runtime::unifiedtimestamp())
                                              .WithHeight(nUH_gb)
                                              .WithLastTemplateUnifiedHeight(nUH_gb)
                                              .WithHashLastBlock(hashBest_gb);
                            }, 1);
                    }
                ...
                /* Cache MISS — proceed with normal new_block() path */
                ...
                if(!gbResult.fSuccess)
                {
                    if(gbResult.eReason == GetBlockPolicyReason::RATE_LIMIT_EXCEEDED)
                    {
                        ...
                        /* Track consecutive rate-limit violations — brief lock for mutation. */
                        uint32_t nStrikes;
                        uint32_t nSessionId_log;
                        {
                            LOCK(MUTEX);
                            nStrikes = ++m_rateLimit.nConsecutiveRateLimitStrikes;
                            nSessionId_log = context.nSessionId;
                        }
                        debug::log(1, FUNCTION,
                            "GET_BLOCK consecutive rate-limit strike #", nStrikes,
                            "/", RateLimitConfig::MAX_CONSECUTIVE_RATE_LIMIT_STRIKES,
                            " session=", nSessionId_log,
                       ...
                       !LLP::IsDefaultSessionReady())
                    {
                        ...
                        debug::error(FUNCTION,
                            ...
                            " to miner (session=", nSessionId_snap,
                            " peer=", GetAddress().ToStringIP(), ")."
                ...
                if(gbResult.vPayload.size() > METADATA_SIZE)
                {
                    std::vector<uint8_t> vRawBlock(
                        gbResult.vPayload.begin() + METADATA_SIZE,
                        gbResult.vPayload.end());
                    GlobalTemplateCache::Get().CacheTemplate(
                        gbResult.nBlockChannel, nCurrentUnifiedHeight,
                        vRawBlock, gbResult.nBlockBits);
                }
            ...
            if(PACKET.HEADER == SUBMIT_BLOCK)
            {
                ...
                if(!ctxSnap.fEncryptionReady || ctxSnap.vChaChaKey.empty())
                {
                    ...
                    debug::error(FUNCTION, "   fEncryptionReady: ", ctxSnap.fEncryptionReady ? "true" : "false");
                    debug::error(FUNCTION, "   vChaChaKey size: ", ctxSnap.vChaChaKey.size(), " (expected: 32)");
                ...
                debug::log(0, "   From: ", GetAddress().ToStringIP());
                debug::log(0, "   Session ID: 0x", std::hex, ctxSnap.nSessionId, std::dec);
                debug::log(0, "   Authenticated: ", ctxSnap.fAuthenticated ? "YES" : "NO");
                debug::log(0, "   Encryption ready: ", ctxSnap.fEncryptionReady ? "YES" : "NO");
                debug::log(0, "   ChaCha key size: ", ctxSnap.vChaChaKey.size(), " bytes");
                debug::log(0, "   Channel: ", ctxSnap.nChannel);
                ...
                debug::log(2, FUNCTION, "SUBMIT_BLOCK from ", GetAddress().ToStringIP(),
                           " channel=", ctxSnap.nChannel, " sessionId=", ctxSnap.nSessionId,
                           " size=", PACKET.DATA.size(),
                           " bound_reward_hash=", FullHexOrUnset(ctxSnap.hashRewardAddress));
                ...
                if(PACKET.DATA.size() >= FALCON_MIN_SIZE)
                {
                        ...
                        {
                            std::lock_guard<std::mutex> lock(SESSION_MUTEX);
                            auto it = mapSessionKeys.find(ctxSnap.nSessionId);
                        ...
                        debug::log(0, "   Size: ", vSessionPubKey.size(), " bytes (expected: ",
                                   (ctxSnap.nFalconVersion == LLC::FalconVersion::FALCON_1024)
                                       ? FalconConstants::FALCON1024_PUBKEY_SIZE    // 1793
                                       : FalconConstants::FALCON512_PUBKEY_SIZE,    // 897
                                   " for ",
                                   (ctxSnap.nFalconVersion == LLC::FalconVersion::FALCON_1024) ? "Falcon-1024" : "Falcon-512",
                                   ")");
                        ...
                        else
                        {
                                                     ...
                                                     FalconConstants::LENGTH_FIELD_SIZE)
                            {
                                ...
                                if(!fDecrypted)
                                {
                                    ...
                                    debug::log(0, FUNCTION, "- authenticated: ", YesNo(ctxSnap.fAuthenticated));
                                    debug::log(0, FUNCTION, "- connection address: ", GetAddress().ToStringIP());
                                    debug::log(0, FUNCTION, "- bound reward hash: ", FullHexOrUnset(ctxSnap.hashRewardAddress));
                                ...
                                debug::log(2, FUNCTION, "✅ Disposable ",
                                          DetectedFalconVersionString(ctxSnap.fFalconVersionDetected, ctxSnap.nFalconVersion),
...

                {
                     ...
                     */
                    {
                        const CanonicalChainState& snap = ctxSnap.canonical_snap;
                    ...
                    if(ctxSnap.hashGenesis != 0)
                    {
                        ColinMiningAgent::Get().on_canonical_snap_updated(
                            ctxSnap.hashGenesis.SubString(8), nSnapAgeMs, fSnapStale);
                    }
                }
                ...
                if(SequenceDiagnosticsEnabled())
                {
                    ...
                    debug::log(0, FUNCTION,
                        ...
                        " canonical.snap.stale=", (ctxSnap.canonical_snap.is_canonically_stale() ? "yes" : "no"),
                        " canonical.unified=", ctxSnap.canonical_snap.canonical_unified_height);
                }
                ...
                else
                {
                    ...
                    {
                        ColinMiningAgent::Get().on_block_submitted(
                            ctxSnap.hashGenesis.SubString(8), pTritium->nChannel, true, "");
                    }
                }

                /* Update context timestamp — brief lock for mutation */
                {
                    LOCK(MUTEX);
                    context = context.WithTimestamp(runtime::unifiedtimestamp());
                }
            ...
            if(PACKET.HEADER == GET_HEIGHT)
            {
                ...
                /* Update context timestamp and height — brief lock for mutation */
                {
                    LOCK(MUTEX);
                    context = context.WithTimestamp(runtime::unifiedtimestamp())
            ...
            if(PACKET.HEADER == GET_ROUND)
            {
                ...
                if(fTemplateStale)
                {
                    /* Check the global template cache first.  TryAttachBlockTemplate()
                     * (called from the PUSH path on the FLUSH_THREAD) stores a fresh
                     * template here immediately after building it.  If the cache has a
                     * valid entry for the current unified height we can serve it without
                     * calling new_block(), eliminating the LLD I/O contention. */
                    std::vector<uint8_t> vPayload;
                    bool fTemplateSent = false;

                    auto cached = GlobalTemplateCache::Get().GetCachedTemplate(
                        nChannel_snap, snap.nUnifiedHeight);
                    if(cached.fValid)
                    {
                        /* Cache HIT — rebuild payload from cached block bytes and fresh
                         * metadata.  The metadata (unified height, channel height, nBits)
                         * is re-serialized so the payload matches current chain state.
                         * This avoids calling new_block() and hitting LLD again. */
                        std::vector<uint8_t> vMetadata = RoundStateUtility::SerializeTemplateMetadata(
                            snap.nUnifiedHeight, nChannelHeight, cached.nBits);
                        ...
                        debug::log(2, FUNCTION, "GET_ROUND auto-send: cache HIT channel=",
                                   nChannel_snap, " unified=", snap.nUnifiedHeight,
                                   " size=", vPayload.size(), "B — skipping new_block()");
                    }
                    else
                    {
                        /* Cache MISS — create block template with retry logic (up to 3 attempts) */
                        ...
                        else
                        {
                            try {
                                ...
                                if(!vBlockData.empty())
                                {
                                    ...
                                    /* Store in cache for future requests at this height */
                                    GlobalTemplateCache::Get().CacheTemplate(
                    ...
                    if(!vPayload.empty())
                    {
                        ...
                        debug::log(2, FUNCTION, "✅ BLOCK_DATA auto-sent (",
                                   vPayload.size(), " bytes) channel=", nChannel_snap,
                                   " unified=", snap.nUnifiedHeight);
             ...
             * handler at line ~774, BEFORE the per-opcode handlers.
            ...
            if(PACKET.HEADER == OpcodeUtility::Stateless::SESSION_STATUS)
            {
                ...
                if(SessionStatusUtility::IsDegraded(req) &&
                    fAuth_snap && (nChannel_snap == 1 || nChannel_snap == 2))
                {
            ...
            if(PACKET.HEADER == ColinDiagOpcodes::PONG_DIAG_STATELESS)
            {
                ...
                {
                    /* Brief lock: snapshot hashGenesis for diagnostic reporting */
                    ...
                    ColinMiningAgent::Get().on_pong_received(
                        hashGenesis_snap != 0 ? hashGenesis_snap.SubString(8) : "",
                        PACKET.DATA);
                }
                return true;
            }
             ...
             * Snapshot context under brief lock, process unlocked, then brief lock for mutations. */
            MiningContext ctxForProcessor;
            {
                LOCK(MUTEX);
                ctxForProcessor = context;
            }
            ProcessResult result = StatelessMiner::ProcessPacket(ctxForProcessor, PACKET);

            /* Update context if successful — acquire MUTEX for the duration of the
             * context mutation pipeline (auth, ChaCha derivation, session registration,
             * persistence).  These are fast local operations with no calls to functions
             * that acquire MUTEX internally, so a single scoped lock is safe and simple. */
            if(result.fSuccess)
            {
                LOCK(MUTEX);
    ...
    void StatelessMinerConnection::TryAttachBlockTemplate()
    {
        ...
        /* Cache template so GET_ROUND auto-send and GET_BLOCK can reuse it
         * without calling new_block() and hitting LLD again. */
        GlobalTemplateCache::Get().CacheTemplate(
            pBlock->nChannel, nUnifiedHeight, vBlockData, pBlock->nBits);
