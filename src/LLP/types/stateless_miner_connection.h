...
namespace LLP
{
    ...
    public:
         ...
         *  throttle in SendChannelNotification / SendStatelessTemplate) and reassigns
         *  m_get_block_cooldown to the "never triggered" state (Ready() returns true
         *  immediately) so the miner's first GET_BLOCK after the heartbeat push is not
         *  deferred by the 1-second per-connection floor.
         *
