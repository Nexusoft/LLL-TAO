...
namespace LLP
{
    ...
    void LookupNode::Event(uint8_t EVENT, uint32_t LENGTH)
    {
        ...
        if(EVENT == EVENTS::HEADER)
        {
            ...
            if(fDDOS.load() && Incoming())
            {
                ...
                /* Incoming connection should always be sending requests. */
    ...
    bool LookupNode::ProcessPacket()
    {
        ...
        /* Check our requests first. */
        if(REQUEST::VALID(INCOMING.HEADER))
        {
        ...
        /* Now we can guarantee thisw is a response. */
        else
        {
            switch(INCOMING.HEADER)
            {
                ...
                default:
                    return debug::error(FUNCTION, "invalid packet header");
            }
        }
