...
public:
    ...
    uint8_t get(uint64_t nPos) const
    {
        if(nPos >= vchData.size())
            throw debug::exception(FUNCTION, "get out of bounds ", nPos);
    ...
    void seek(int64_t nSeek, uint8_t nFlags = STREAM::CURSOR) const
    {
        ...
        if(nFlags == STREAM::CURSOR)
        {
            if(nReadPos + nSeek > vchData.size())
                throw debug::exception(FUNCTION, "out of bounds ", nSeek);
    ...
    const BaseStream& read(char* pch, uint64_t nSize) const
    {
        ...
        if(nReadPos + nSize > vchData.size())
            throw debug::exception(FUNCTION, "reached end of stream ", nReadPos);
