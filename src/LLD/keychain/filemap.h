    ...
    **/
    ...
    public:
            ...
            while(true)
            {
                ...
                while(nIterator < nCurrentFileSize)
                {
                    ...
                    if(cKey.Ready())
                    {
                        ...
                        debug::log(5, FUNCTION "State: %u Length: %u File: %u Location: %u Key: %s", __PRETTY_FUNCTION__, cKey.nState, cKey.nLength, mapKeys[nBucket][vKey].first, mapKeys[nBucket][vKey].second, HexStr(vKey.begin(), vKey.end()).c_str());
        ...
        bool Put(SectorKey cKey) const
        {
            ...
            /* Debug Output of Sector Key Information. */
            debug::log(4, FUNCTION "State: %s | Length: %u | Location: %u | File: %u | Sector File: %u | Sector Size: %u | Sector Start: %u | Key: %s | Current File: %u | Current File Size: %u", __PRETTY_FUNCTION__, cKey.nState == READY ? "Valid" : "Invalid", cKey.nLength, mapKeys[nBucket][cKey.vKey].second, mapKeys[nBucket][cKey.vKey].first, cKey.nSectorFile, cKey.nSectorSize, cKey.nSectorStart, HexStr(cKey.vKey.begin(), cKey.vKey.end()).c_str(), nCurrentFile, nCurrentFileSize);
        ...
        bool Get(const std::vector<uint8_t> vKey, SectorKey& cKey)
        {
            ...
            if(mapKeys[nBucket].count(vKey))
            {
                ...
                /* Debug Output of Sector Key Information. */
                debug::log(4, FUNCTION "State: %s | Length: %u | Location: %u | File: %u | Sector File: %u | Sector Size: %u | Sector Start: %u | Key: %s", __PRETTY_FUNCTION__, cKey.nState == READY ? "Valid" : "Invalid", cKey.nLength, mapKeys[nBucket][vKey].second, mapKeys[nBucket][vKey].first, cKey.nSectorFile, cKey.nSectorSize, cKey.nSectorStart, HexStr(vKey.begin(), vKey.end()).c_str());
