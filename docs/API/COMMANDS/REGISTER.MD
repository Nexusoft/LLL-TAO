...
```
register/list/finance:account,finance:trust/total/sum limit=none where='results.token=0'
```
...
```
register/list/finance:accounts,finance:trust/owner,type,total sort=total order=desc page=0 where='results.token=0'
```
