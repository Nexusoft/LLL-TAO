...
## Appendix C: Prime Sign/Finalization Refactor (PR #391 replacement)
...
- Unlocks the mining sigchain credentials.
- Calls `block.GenerateSignature(key)` which signs `SignatureHash()` — covering the
  final `nNonce`, `vOffsets`, and `nTime`.
- Must be called after `nNonce` and `vOffsets` are applied and before
  `ValidateMinedBlock()` / `AcceptMinedBlock()`.

### Why Network Peers Do Not Need Stateless Node

The Falcon signature in the stateless transport authenticates the **session payload**
(miner identity + block body integrity) between the miner and the node. This is a
transport-layer mechanism.

The `vchBlockSig` wallet signature is the **consensus-layer** proof that the block
was produced by an authorised Nexus sigchain holder. It is generated by the node
using the producer transaction's signing key and is verifiable by any peer using the
public key in the `producer` transaction — no knowledge of the stateless protocol
is required.

The final relayed block is a standard canonical `TritiumBlock` with all
consensus-visible fields intact, indistinguishable from a block produced by the
upstream local-miner path.

### Updated Prime Submit Flow (Stateless Lane)
