...
TEST_CASE( "Validation Script Tests", "[operation]" )
{
    ...
    ////////////////////OBJECT REGISTERS
    {
...

      {
       ...
       ssOperation.SetNull();
       ssOperation << uint8_t(OP::TYPES::UINT256_T) << hashObject << uint8_t(OP::REGISTER::VALUE) << std::string("token_address")
       ...
       ssOperation.SetNull();
       ssOperation << uint8_t(OP::TYPES::STRING) << strObject << uint8_t(OP::CRYPTO::SK256) << uint8_t(OP::REGISTER::VALUE) << std::string("token_address")
                   << uint8_t(OP::EQUALS) << uint8_t(OP::TYPES::UINT256_T) << uint256_t(0);
       {
           Validate script = Validate(ssOperation, tx);
           REQUIRE(script.Execute());
