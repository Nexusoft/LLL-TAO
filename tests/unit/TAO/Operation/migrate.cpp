...
/*
 * Stake migration disabled after transaction version 1.
 * These tests will test correct migration for version 1, or rejection for version 2+.
 *
 * All these migration tests should be considered deprecated and can be removed at a later time.
 */
...
TEST_CASE( "Migrate Operation Test - Genesis coinstake", "[operation]")
{
    ...
    //Create OP::MIGRATE to credit the legacy send and complete trust key migration (genesis only trust key)
    {
        ...
        //process results depend on transaction version
        ...
        if(nCurrent > 2 || (nCurrent == 2 && TAO::Ledger::TransactionVersionActive(runtime::timestamp(), nCurrent)))
        {
            ...
            {
                //check trust key not migrated
                REQUIRE_FALSE(LLD::Legacy->HasTrustConversion(hashTrust));

                //check trust account not indexed
                REQUIRE_FALSE(LLD::Register->HasTrust(hashGenesis));

                //get the un-indexed trust register using hashRegister
                TAO::Register::Object trust;
                REQUIRE(LLD::Register->ReadState(hashRegister, trust));
    ...
    //Skip test for duplicate trust key migration if tx v2+ (initial migration will have failed above)
    uint32_t nCurrent = TAO::Ledger::CurrentTransactionVersion();
...
TEST_CASE( "Migrate Operation Test - Invalid OP::MIGRATE tests", "[operation]")
{
    ...
    /* Skip invalid OP::MIGRATE tests if not testing tx version 1 */
    uint32_t nCurrent = TAO::Ledger::CurrentTransactionVersion();
    if(nCurrent == 1 || (nCurrent == 2 && !TAO::Ledger::TransactionVersionActive(runtime::timestamp(), nCurrent)))
    {
