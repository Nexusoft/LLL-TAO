...
TEST_CASE("Falcon Constants Validation", "[falcon][constants]")
{
    ...
    {
        // AUTH_RESPONSE_MAX uses Falcon-1024 (default):
        // pubkey_len(2) + pubkey(1793) + timestamp(8) + sig_len(2) + sig(1577) = 3382 bytes
        REQUIRE(AUTH_RESPONSE_MAX == 3382);
        REQUIRE(AUTH_RESPONSE_MAX == AUTH_RESPONSE_FALCON1024_MAX);
        
        // AUTH_RESPONSE_ENCRYPTED_MAX = 3382 + 28 = 3410 bytes
        REQUIRE(AUTH_RESPONSE_ENCRYPTED_MAX == 3410);
        REQUIRE(AUTH_RESPONSE_ENCRYPTED_MAX == AUTH_RESPONSE_MAX + CHACHA20_OVERHEAD);
        
        // AUTH_RESPONSE_WITH_GENESIS_MAX = 3410 + 32 = 3442 bytes
        REQUIRE(AUTH_RESPONSE_WITH_GENESIS_MAX == 3442);
